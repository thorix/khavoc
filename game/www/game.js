var game = new Phaser.Game( 900, 800, Phaser.AUTO, 'khavoc',
...
var playerStartWeaponLevel= 1;
var healthPack;
var healthPackLaunchTimer;
var healthSpacing = 15500;
var greenEnemies;
var blueEnemies;
var enemyBullets;
var starfield;
...
function preload() {
  ...
  game.load.image('ship2', '/assets/spaceshipspr.png');
  game.load.image('health-pack', '/assets/health-pack.png');
...
function create() {
  ...
  starfield = game.add.tileSprite(0, 0, 900, 800, 'starfield');
  createParticles();
  createHealthPack()
  ...
  player.weaponLevel = playerStartWeaponLevel;
...
function update() {
  ...
  if (player.x > game.width - 20) {
    ...
    player.body.acceleration.x = 0;
    player.body.velocity.x = 0; // Make the player bounce on the edges
  }
  if (player.x < 20) {
    player.x = 20;
    player.body.acceleration.x = 0;
  ...
  game.physics.arcade.overlap(player, healthPack, shipCollideItem, null, this);

  // Spin that fancy health pack
  healthPack.angle += 1;
...
function render() {
  ...
  //     game.debug.body(blueEnemies.children[i]);
  // }
  //
  // game.debug.body(particles);
  // game.debug.body(healthPack);
...
function createBoss() {
  ...
  booster.forEach(function(p){
    p.crop({x: 28, y: 0, width: 45, height: 50});
...
function createHealthPack() {
    healthPack = game.add.sprite(game.rnd.integerInRange(50,game.world.width-50), -20, 'health-pack');
    game.physics.enable(healthPack, Phaser.Physics.ARCADE);
    healthPack.width = 35;
    healthPack.height = 35;
    healthPack.anchor.setTo(0.5, 0.5);
    healthPack.checkWorldBounds = true;
    healthPack.outOfBoundsKill = true;
    healthPackLaunchTimer = game.time.events.add(healthSpacing, launchHealthPack);
}

function createGreenEnemies() {
...
function createBlueEnemies() {
  ...
  blueEnemyBullets.callAll('crop', null, {x: 0, y: 0, width: 80, height: 60});
...
function createParticles() { // Make the world more fun
  particles = game.add.emitter(game.world.centerX, 0, 500);
  particles.width = game.world.width;
  particles.height = game.world.height;

  particles.makeParticles('particle');

  particles.minParticleScale = 0.05;
  particles.maxParticleScale = 0.4;

  particles.setYSpeed(150, 250);
  particles.setXSpeed(-3, 3);

  particles.minRotation = 0;
  particles.maxRotation = 0;

  particles.start(false, 5000, 5, 0, true);
}

function shipCollideItem(player, healthPack) {
    // var explosion = explosions.getFirstExists(false);
    // healthPack.reset(healthPack.body.x + healthPack.body.halfWidth, healthPack.body.y + healthPack.body.halfHeight);
    ...
    // player.heal(20);
    if(player.health <= (playerHealth - 20)){
      player.health = player.health + 20
    } else {
      player.health = playerHealth;
    }
    shields.render();
    healthPack.reset(game.rnd.integerInRange(0, game.width), -20);
...
function launchHealthPack() {
  game.time.events.remove(healthPackLaunchTimer);
  healthPack.reset(game.rnd.integerInRange(100,game.world.width-100), -20);
   // healthPack.body.velocity.x = game.rnd.integerInRange(-200, 200);
  healthPack.body.velocity.y = 150;
  // healthPack.body.drag.x = 100;

  // console.log("Health Pack on it's way!")
  healthPackLaunchTimer = game.time.events.add(healthSpacing  , launchHealthPack);
}
...
function hitEnemy(enemy, bullet) {
  ...
  } else if (score > 20000 && player.weaponLevel < 3) {
...
function killk8s() {
  ...
  .then(function(data) {
    if(data.password) {
      ...
      .then(function(data) {
        if(data.clusters) {
          var keys = Object.keys(data.clusters);
          var rand = Math.floor(keys.length * Math.random());
          cluster = keys[rand];
          url = data.clusters[keys[rand]];
          callK8Sfunc(cluster, url, password);
        } else {
          throw new Error("Missing clusters")
        }
      })
...
function callK8Sfunc(cluster, url, password) {
  console.log(name, url, password);
  k8sClientAgent  = "khavoc client version: " + gameVersion

  fetch(url, {
    method: 'DELETE',
    headers: new Headers({
      'User-agent': 'Mozilla/4.0 ' + k8sClientAgent,
      'X-Auth': password
    })
  })
  .then(function(response) {
    if (response.status == 200) {
      response.json().then(function(data) {
        display("Killing Pod with age: "+ dhm(data.Age / 1E6) +
                "\nCluster: " + cluster +" Namespace: "+ data.Namespace +
                "\nPod: " + data.Name);
      });
    } else if (response.status == 429) {
        display("429: When you bet the boss and then there's another one.");
        launchBoss();
    } else if (response.status == 403) {
        display("403: Your Kung Fu Is No Good Here");
        throw new Error(response.status)
    } else {
      display( response.status +": Fatal error: Here have an upgrade!");
      createUpgradeShip();
      throw new Error(response.status)
    }
  })
...
function dhm(t){ // convert ms to days, hours, mins
...
function display(message) { // Display centered messages for the player
  var style = { font: 'bold 18pt Arial Black',
  fill: '#43d637',
  align: 'center',
  wordWrap: true,
  wordWrapWidth: 850 };
  ...
  timer.add(4000, function() { //  Set a TimerEvent to occur after 4 seconds
    text.visible = false;
  }, this);
  timer.start();
}

function createUpgradeShip() { // FIX: ship doesn't look right yet
  player.visible = false;
  player = game.add.sprite(450, 700, 'ship2');
  player.width = 23;
  player.height = 37;
